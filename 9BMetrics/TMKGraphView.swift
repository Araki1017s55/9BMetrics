...
class TMKGraphView: UIView {
    ...
    @IBAction func clamp(_ src: UITapGestureRecognizer)
    {
        ...
        if myPt.x > self.selectionLeft && myPt.x < self.selectionRight && myPt.y > ( self.topMargin) && myPt.y < (self.bounds.size.height - self.bottomMargin)
        {
            ...
            if self.clampDown {
                ...
                self.statsField.text = stats
                self.statsField.isHidden = false
    ...
    func computeDataForSeries()
    {
        ...
        if let ds = self.dataSource {
            ...
            yminr = [CGFloat](repeating: 0.0 , count: nseries-1)
            ymaxr = [CGFloat](repeating: 100.0 , count: nseries-1)
    ...
    func rightViewPointFromTrackPoint(_ pt:CGPoint, serie : Int) -> CGPoint {
    ...
    func viewPointFromTrackPoint(_ pt:CGPoint, serie : Int) -> CGPoint
    {
        var  myPt = CGPoint(x:0, y:0)
        let height = self.bounds.size.height
    ...
    func addGearButton()
    {
        ...
        let clearImage = UIImage(named:"share")?.withRenderingMode(UIImageRenderingMode.alwaysTemplate) // era nakedGear
        ...
        but.addTarget(self, action:#selector(TMKGraphView.openGear(_:)), for:UIControlEvents.touchUpInside)
 ...
 //       sCons = NSLayoutConstraint(item:tf, attribute:NSLayoutAttribute.Width, relatedBy:NSLayoutRelation.Equal, toItem:nil, attribute:NSLayoutAttribute.NotAnAttribute, multiplier:1.0, constant:80.0)
    ...
    func minuteStep(_ rawStep : CGFloat) -> CGFloat{
